...
export const Sidebar: React.FC<SidebarProps> = ({ tasks, onAddTask, onToggleTask, onDeleteTask, onUpdateTaskTitle, history }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-b from-white/15 to-white/5 backdrop-blur-md overflow-hidden">
      {/* Content Area */}
      <div className="flex-1 overflow-y-auto px-4 pt-4 pb-4 custom-scrollbar relative">
        {activeTab === 'todo' ? (
          <div className="space-y-4">
            <form onSubmit={addTask} className="relative group">
              <input
                ...
                className="w-full glass-input rounded-xl px-5 py-4 text-base text-white placeholder-white/50 bg-black/30 backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-white/30 transition-all"
              />
              <button
                type="submit"
              ...
              {tasks.map((task) => (
                <div
                  ...
                  className={`group flex items-center gap-4 p-4 rounded-xl border transition-all duration-200 ${task.completed
                    ? 'bg-emerald-500/10 border-emerald-500/20 opacity-70 hover:opacity-90'
                    : 'bg-black/25 border-white/15 hover:bg-black/35 hover:border-white/25 shadow-sm backdrop-blur-sm'
                    }`}
                >
                  <button
                  ...
                  {editingId === task.id ? (
                    ...
                    <span className={`flex-1 text-base font-medium drop-shadow-md transition-colors ${task.completed ? 'text-white/60 line-through' : 'text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]'}`}>
                      {task.title}
                    </span>
                  )}
          ...
          <div className="space-y-3">
            ...
            {[...history].reverse().map((session) => (
              <div key={session.id} className="flex items-center justify-between p-3.5 rounded-xl bg-black/25 border border-white/15 hover:bg-black/35 backdrop-blur-sm transition-colors">
                <div>
      ...
      <div className="p-4 pt-2">
        <div className="flex p-1 bg-black/30 backdrop-blur-md rounded-xl border border-white/20">
          <button
            onClick={() => setActiveTab('todo')}
            className={`flex-1 py-3 rounded-lg flex items-center justify-center gap-2 text-sm font-medium transition-all duration-300 ${activeTab === 'todo' ? 'bg-white text-black shadow-lg scale-[1.02]' : 'text-white/60 hover:text-white hover:bg-white/5'}`}
          >
            <ListTodo size={18} />
            <span>To-Do</span>
          </button>
          <button
            ...
            className={`flex-1 py-3 rounded-lg flex items-center justify-center gap-2 text-sm font-medium transition-all duration-300 ${activeTab === 'history' ? 'bg-white text-black shadow-lg scale-[1.02]' : 'text-white/60 hover:text-white hover:bg-white/5'}`}
          >
            <Clock size={18} />
            <span>History</span>
          </button>
